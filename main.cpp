...
public:
  ...
  TransformerBlock(huggingface::safetensors::safetensors_t param, int b)
      : qkv(0, 3 * n_embd),
  ...
  */
  void operator()(Eigen::VectorXf& x) {
    ...
    VectorXf qkv_row = w_attn1.transpose() * (x.array() - x.mean()).matrix().normalized()  + b_attn1;
    ...
    Eigen::VectorXf h = w_mlp1.transpose() * (x.array() - x.mean()).matrix().normalized() + b_mlp1;
...
int main() {
  ...
  auto param = huggingface::safetensors::deserialize(model);
  ...
  std::vector<int> prompt = {36235, 39141, 18765, 1143, 326, 9061, 561, 530,
                             1110,  1716};
  ...
  for (int posn = 0; posn < total; posn++) {
    ...
    Eigen::VectorXf final = (x.array() - x.mean()).matrix().normalized().array() * w_ln.array() + b_ln.array();
